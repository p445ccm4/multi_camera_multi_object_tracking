...
parser = argparse.ArgumentParser(description="check single camera accuracy")
parser.add_argument("folder", type=str, help="Path to the folder containing images and matrices from Camera")
...
# parser.add_argument("image", type=str, help="Path to the images from Camera that contain the origin")
args = parser.parse_args()

folder = args.folder
# Load the saved array
matrix = np.load(os.path.join(folder, 'internal_matrix.npy'))
r_vec = np.load(os.path.join(folder, 'rotation_vector.npy'))
t_vec = np.load(os.path.join(folder, 'translation_vector.npy'))
distortion = np.load(os.path.join(folder, 'distortion.npy'))
...
if __name__ == "__main__": 
    # Read the image
    original_img = cv2.imread(os.path.join(folder, 'calibration1.jpg'))
    # original_img_undist = cv2.undistort(original_img, matrix, distortion)
    original_img_undist_add_origin = draw_axes(original_img, matrix, r_vec, t_vec)
    img = original_img_undist_add_origin.copy()
