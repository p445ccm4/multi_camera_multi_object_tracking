...
parser.add_argument("M_sec2world", help="matrix from cam2 camera coordinate to cam1 world coordinate")
...
M_sec2world = np.load(args.M_sec2world)
...
def draw_circle(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        cam2_camera_coords = world_to_camera_coordinate(cam2_world_coords, r_vec, t_vec)
        # print("camera_coords: ", camera_coords)
        cam2_camera_coords = np.append(cam2_camera_coords, 1)
        cam1_world_coords = np.linalg.inv(M_sec2world) @ cam2_camera_coords
        cv2.putText(img, f'{(str(cam1_world_coords[0])),(str(cam1_world_coords[1]))}', (x, y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
        print(f'cam1_world_coords: {cam1_world_coords}')
